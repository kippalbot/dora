#!/usr/bin/env python3
"""
Chinese Voice Verification Test for Kokoro TTS.
Tests all Chinese male and female voices with performance comparison.
"""
...
# Test text - Chinese passage
DEFAULT_TEXT = """我们说中国式现代化是百年大战略，这又分为三个阶段。第一个阶段，我们先用30年时间建成了独立完整的工业体系和国民经济体系；再用40年，到2021年，全面建成了小康社会。我们现在正处于第三个阶段。"""


def test_voice(voice_id: str, voice_name: str, text: str, lang_code: str = 'z') -> Dict:
    """Test a single voice and return metrics.

    Args:
        voice_id: Voice identifier (e.g., zm_yunxi)
        voice_name: Human-readable voice name
        text: Text to synthesize
        lang_code: Language code ('z' for Chinese)
    ...
    # Save audio
    output_dir = Path("tts_output/chinese_voices")
...
def test_all_voices(voice_type: str = 'all', text: str = None) -> Dict:
    """Test all Chinese voices or specific gender.
    ...
    """
    print("=" * 80)
    print("CHINESE VOICE VERIFICATION TEST - KOKORO TTS")
    print("=" * 80)

    test_text = text or DEFAULT_TEXT
    print(f"\nTest text: {len(test_text)} characters")
    print("-" * 60)
    print(test_text[:100] + "..." if len(test_text) > 100 else test_text)
    print("-" * 60)
    ...
    if voice_type in ['male', 'all']:
        ...
        for voice_id, voice_name in CHINESE_VOICES['male'].items():
    ...
    if voice_type in ['female', 'all']:
        ...
        for voice_id, voice_name in CHINESE_VOICES['female'].items():
            result = test_voice(voice_id, voice_name, test_text)
...
def print_summary(results: Dict):
    ...
    print("=" * 80)
    print(f"{'Voice ID':<15} {'Type':<8} {'RTF':<8} {'Time (s)':<10} {'Speed':<12} {'Status':<10}")
    print("-" * 80)

    for voice_id, result in results.get('male_voices', {}).items():
        if result['status'] == 'success':
            print(f"{voice_id:<15} {'Male':<8} {result['rtf']:<8.2f} {result['synthesis_time']:<10.3f} "
                  f"{result['chars_per_second']:<12.1f} {'✓ Success':<10}")
        else:
            print(f"{voice_id:<15} {'Male':<8} {'N/A':<8} {'N/A':<10} {'N/A':<12} {'✗ Failed':<10}")

    for voice_id, result in results.get('female_voices', {}).items():
        if result['status'] == 'success':
            print(f"{voice_id:<15} {'Female':<8} {result['rtf']:<8.2f} {result['synthesis_time']:<10.3f} "
                  f"{result['chars_per_second']:<12.1f} {'✓ Success':<10}")
        else:
            print(f"{voice_id:<15} {'Female':<8} {'N/A':<8} {'N/A':<10} {'N/A':<12} {'✗ Failed':<10}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test all Chinese voices in Kokoro TTS",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        epilog="""
Examples:
  # Test all Chinese voices (male and female)
  ...
  python test_chinese_voices.py --text "你好世界，这是测试。"
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Custom Chinese text to synthesize (optional)'
    )

    parser.add_argument(
        '--save-json',
        type=str,
        default='tts_output/chinese_voices_results.json',
